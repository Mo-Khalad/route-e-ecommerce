...
const FormComponentBM = (
  ...
  if( token === undefined && url === "api/v1/auth/signin" ) {
   ...
   ( responsive?.data?.message !== undefined || responsive.status === 200 || responsive.status === 201)&&
      (responsive?.data?.message === "success" || responsive.status === 200 || responsive.status === 201 ) && navigate(success) 
      fetchCart()
  };
  return (
    <>
        ...
        {(formik) => {
          return (
            <Form>
              ...
              >
                {formik.isSubmitting ? <ScaleLoader 
                color="#d9dddf"
                  height="25px"
                /> : ` ${btn} `
                }
